...
def complex_interp(ordinate, abscissa_interp, abscissa):
    ...
    ordinate_interp_real = np.interp(np.array(abscissa_interp),
            np.array(abscissa), np.real(np.array(ordinate)))

    ordinate_interp_imag = np.interp(np.imag(abscissa_interp),
            np.imag(abscissa), np.imag(np.array(ordinate)))
...
def build_cwt(timeseries, max_scale=256, mother_freq=2, Norm=True, fmin=1000.0,
    ...
    peakidx = np.argmax(abs(timeseries.data))
    midpoint = int(0.5*maplen)
    paddata[midpoint-peakidx:midpoint] = timeseries.data[:peakidx]
    paddata[midpoint:midpoint+len(timeseries.data[peakidx:])] = timeseries.data[peakidx:]
